...
class DDCB(nn.Module):
    ...
    def forward(self, x):
        ...
        x1_raw = self.conv1(x)
        #print(x1_raw.shape)
        x1 = torch.cat([x, x1_raw], 1) # les concaténations se font suivant les lignes (WxH -> 2WxH)
        #print(x1.shape)
...
class DenseScaleNet(nn.Module):
    def __init__(self, load_model='', load_weights=False): # c'est quoi load_model?
        super(DenseScaleNet, self).__init__()
        self.load_model = load_model # ça sert à quoi ??
        # si je veux remplacer VGG par un autre feature extractor, il faut changer ça c'est sûr
        self.features_cfg = [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512] #c'est les 10 layers de VGG-16
        #self.features_cfg = [64, 64, 64, 64, 64, 128, 128, 128, 128, 256, 256, 256]
    ...
    def _initialize_weights(self):
        ...
        if not self.load_model: # load_model doit être un booléen donc?
